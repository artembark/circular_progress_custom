...
class CocktailCircularProgressIndicator extends StatefulWidget {
  final Color color;
  const CocktailCircularProgressIndicator({Key? key, this.color: Colors.white})
      : super(key: key);

  @override
  State<StatefulWidget> createState() =>
      _CocktailCircularProgressIndicatorState();
}

class _CocktailCircularProgressIndicatorState
    extends State<CocktailCircularProgressIndicator>
...
class CircularProgressIndicatorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final innerRadius = _CocktailCircularProgressIndicatorState.SIZE / 2 -
        _CocktailCircularProgressIndicatorState.WIDTH;

    final phaseAngle = (value * 2 * pi);
    final currentAngle = phaseAngle + (value * 2 * pi); // - pi / 2;
    final sweepAngle =
        (value <= 0.5 ? (value * 2 * pi) : ((1 - value) * 2 * pi));
    final startAngle = currentAngle - sweepAngle / 2;

    Path path = Path();
    path.addArc(
        Rect.fromPoints(
            Offset(center.dx - outterOadius, center.dy - outterOadius),
            Offset(center.dx + outterOadius, center.dy + outterOadius)),
        startAngle,
        sweepAngle);
